...
def par_run_KAGE(read_cov_pair):
    ...
    cmd = (
        ...
        f"total=$(bcftools view -H {temp_dir}/{read}_KG_genotyping.vcf.gz | wc -l); homo=$(bcftools view -i 'GT=\"hom\"' {temp_dir}/{read}_KG_genotyping.vcf.gz | wc -l); het=$((total - homo)); echo \"Total variants: $total, Homozygous variants: $homo, Heterozygous variants: $het\" >> {log_file} && "
        f"bcftools view -i 'GT=\"hom\"' {temp_dir}/{read}_KG_genotyping.vcf.gz | bgzip > {temp_dir}/{read}_KG_genotyping_no_homo.vcf.gz && "
