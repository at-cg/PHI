...
mg_idx_t *mg_index_core(gfa_t *g, int k, int w, int b, int n_threads, std::vector<int32_t> &bubble_vertices)
{
	mg_idx_t *gi;
	mg128_v a = {0,0,0};
	int i;
...
mg_idx_t *mg_index(gfa_t *g, const mg_idxopt_t *io, int n_threads, mg_mapopt_t *mo, std::vector<int32_t> &bubble_vertices)
{
	int32_t i, j;
	mg_idx_t *gi;
	...
	gi = mg_index_core(g, io->k, io->w, io->bucket_bits, n_threads, bubble_vertices);
	if (gi == 0) return 0;
